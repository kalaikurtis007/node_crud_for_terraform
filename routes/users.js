...
router.get('/edit/(:id)', function(req, res, next) {
    ...
    dbConn.query('SELECT * FROM usertbl WHERE id = ' + id, function(err, rows, fields) {
        if(err) throw err
...
router.post('/update/:id', function(req, res, next) {
    ...
    if( !errors ) {
        ...
        // update query
        dbConn.query('UPDATE usertbl SET ? WHERE id = ' + id, form_data, function(err, result) {
            //if(err) throw err
            if (err) {
                // set flash message
                req.flash('error', err)
...
router.get('/delete/(:id)', function(req, res, next) {
    ...
    dbConn.query('DELETE FROM usertbl WHERE id = ' + id, function(err, result) {
        //if(err) throw err
        if (err) {
            // set flash message
            req.flash('error', err)
...
router.get('/mock',function(req,res,next){
    ...
    if(param.length>0){
        ...
        req.flash('error', 'User Data was empty');
        res.redirect('/users');
    }
})
